...
namespace MCS.GridMvc.Html
{
    ...
    public class GridHtmlOptions<T> : IGridHtmlOptions<T> where T : class
    {
        ...
        public IGridHtmlOptions<T> CollectionName(string collectionName)
        {
            _source.CollectionName = collectionName;
            return this;
        }
        ...
        public IGridHtmlOptions<T> DefaultSortColumn(string columnName)
        {
            if (string.IsNullOrWhiteSpace(_source.Settings.SortSettings.ColumnName))
                _source.Settings.SortSettings.ColumnName = columnName;
            return this;
        }
        ...
        public IGridHtmlOptions<T> RenderDeleteButton(string deleteActionUrl = null, int columnIndex = -1, string formId = "", string divDataUpdate = "", string onAjaxSuccess = "", string validFunctionCall = "")
        {
            _source.RenderOptions.RenderDeleteButton = true;
            _source.RenderOptions.DeleteActionUrl = deleteActionUrl;
            _source.RenderOptions.ColumnIndex = columnIndex;
            _source.RenderOptions.FormId = formId;
            _source.RenderOptions.DivDataUpdate = divDataUpdate;
            _source.RenderOptions.onAjaxSuccess = onAjaxSuccess;
            _source.RenderOptions.ValidFunctionCall = validFunctionCall;

            return this;
        }

        public IGridHtmlOptions<T> UpdateActionUrl(string url)
        {
            _source.RenderOptions.UpdateActionUrl = url;
            return this;
        }
