...
namespace MCS.GridMvc.Columns
{
    public abstract class GridColumnBase<T> : IGridColumn<T>
    {
        ...
        public bool RenderEnabled(object instance)
        {
            if (RenderConstraint == null)
                return true;
            return RenderConstraint.Invoke((T)instance);
        }
        public IGridColumn<T> Titled(string title, string tag = "")
        {
            Title = title;
            Tag = tag;
            return this;
        ...
        public virtual IGridColumn<T> RenderDataIf(Func<T, bool> constraint)
        {
