    ...
    $.extend(true, plugin, {
        applyFilterValues: function (initialUrl, columnName, values, skip) {
            ...
            self.updateGrid(fullSearch, function () {
                ...
                if (self.jqContainer.find(".grid-page-size option:selected").length > 0) {
                    self.jqContainer.find('.grid-page-size  option:selected').attr("selected", null);
                    self.jqContainer.find('.grid-page-size option[value=' + self.pageSize + ']').attr("selected", "selected");
                }
        ...
        ajaxify: function (options) {
            ...
            if (self.jqContainer.find(".grid-page-size option:selected").length > 0) {
                ...
                self.pageSize = parseInt(self.jqContainer.find(".grid-page-size option:selected").text());
            }
            ...
            self.updateGrid = function (search, callback, renderRowsOnly) {
                var gridQuery = "&";
                gridQuery += "&gridName=" + self.jqContainer.data("gridname");
                //gridQuery += "&param=" + self.param;
                ...
                gridQuery += "&pageSize=" + self.pageSize;
            ...
            self.setupPagerLinkEvents = function () {
                ...
                self.jqContainer.on("change", ".grid-page-size", function (e) {
                    e.preventDefault();
              
                    self.pageSize = $(this).val();


                    var fullSearch = self.loadPagedDataAction;
                    if (fullSearch.indexOf("?") == -1) {
                        fullSearch += "?pageSize=" + self.pageSize;

                    }
                    else {
                        fullSearch = fullSearch.substring(0, fullSearch.indexOf("?"));
                        fullSearch += "?pageSize=" + self.pageSize;

                    }

                    var gridUrl = URI(fullSearch).normalizeSearch().toString();

                    self.updateGrid(gridUrl, function () {
                        if (self.jqContainer.find(".grid-page-size option:selected").length > 0) {
                            self.jqContainer.find('.grid-page-size  option:selected').attr("selected", null);
                            self.jqContainer.find('.grid-page-size option[value=' + self.pageSize + ']').attr("selected", "selected");
                        }
            ...
            self.loadPage = function () {
                ...
                $.post(gridUrl, { param: self.param, __RequestVerificationToken: token})
                    .done(function (response) {
                        gridTableBody.html("");
                        gridTableBody.append(response.Html);
                        if (self.lastPageNum == self.currentPage) {
                            nextPageLink.hide();
                        } else {
                            nextPageLink.show();
                        }
