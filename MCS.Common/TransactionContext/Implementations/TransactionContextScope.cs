...
{
    public class TransactionContextScope : ITransactionContextScope
    {
        public int? tenantId;
        ...
        private TransactionContextScope _parentScope;
        private TransactionContextCollection _TransactionContexts;

        public ITransactionContextCollection TransactionContexts { get { return _TransactionContexts; } }

        public TransactionContextScope(ITransactionContextFactory TransactionContextFactory = null) :
            this(joiningOption: transactionContextScopeOption.JoinExisting, readOnly: false, isolationLevel: null, TransactionContextFactory: TransactionContextFactory)
        { }

        public TransactionContextScope(bool readOnly, ITransactionContextFactory TransactionContextFactory = null)
            : this(joiningOption: transactionContextScopeOption.JoinExisting, readOnly: readOnly, isolationLevel: null, TransactionContextFactory: TransactionContextFactory)
        { }

        public TransactionContextScope(transactionContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel, ITransactionContextFactory TransactionContextFactory = null, int? tenantId = -1)
        {
            if (isolationLevel.HasValue && joiningOption == transactionContextScopeOption.JoinExisting)
                throw new ArgumentException("Cannot join an ambient transactionContextScope when an explicit database transaction is required. When requiring explicit database transactions to be used (i.e. when the 'isolationLevel' parameter is set), you must not also ask to join the ambient context (i.e. the 'joinAmbient' parameter must be set to false).");
            ...
            if (_parentScope != null && joiningOption == transactionContextScopeOption.JoinExisting)
            {
                if (_parentScope._readOnly && !this._readOnly)
                {
                    throw new InvalidOperationException("Cannot nest a read/write transactionContextScope within a read-only transactionContextScope.");
                }

                _nested = true;
                _TransactionContexts = _parentScope._TransactionContexts;
            }
            else
            {
                _nested = false;
                _TransactionContexts = new TransactionContextCollection(readOnly, isolationLevel, TransactionContextFactory);
            }
        ...
        {
            return _TransactionContexts.Commit();
        }

        private Task<int> CommitInternalAsync(CancellationToken cancelToken)
        {
            return _TransactionContexts.CommitAsync(cancelToken);
        }

        private void RollbackInternal()
        {
            _TransactionContexts.Rollback();
        }
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            if (_nested) // The parent scope uses the same TransactionContext instances as we do - no need to refresh anything
                return;

            // OK, so we must loop through all the TransactionContext instances in the parent scope
            ...
            // entities from one TransactionContext instance to another. NHibernate has support for this sort of stuff 
            // but EF still lags behind in this respect. But there is hope: https://entityframework.codeplex.com/workitem/864

            // NOTE: TransactionContext implements the ObjectContext property of the IObjectContextAdapter interface explicitely.
            // So we must cast the TransactionContext instances to IObjectContextAdapter in order to access their ObjectContext.
            // This cast is completely safe.

            foreach (IObjectContextAdapter contextInCurrentScope in _TransactionContexts.InitializedTransactionContexts.Values)
            {
                var correspondingParentContext =
                    _parentScope._TransactionContexts.InitializedTransactionContexts.Values.SingleOrDefault(parentContext => parentContext.GetType() == contextInCurrentScope.GetType())
                    as IObjectContextAdapter;

                if (correspondingParentContext == null)
                    continue; // No TransactionContext of this type has been created in the parent scope yet. So no need to refresh anything for this TransactionContext type.

                // Both our scope and the parent scope have an instance of the same TransactionContext type. 
                // We can now look in the parent TransactionContext instance for entities that need to
                // be refreshed.
                foreach (var toRefresh in entities)
                {
                    ...
                    // already been loaded in the parent TransactionContext's first-level cache (the ObjectStateManager).
                    ...
                    if (contextInCurrentScope.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(toRefresh, out stateInCurrentScope))
                    {
                        ...
                        // Now we can see if that entity exists in the parent TransactionContext instance and refresh it.
                        ...
                        {
                            // Only refresh the entity in the parent TransactionContext from the database if that entity hasn't already been
        ...
        public void Dispose()
        {
            ...
            // Commit / Rollback and dispose all of our TransactionContext instances
            if (!_nested)
            {
                if (!_completed)
                {
            ...
            if (currentAmbientScope != this) // This is a serious programming error. Worth throwing here.
                throw new InvalidOperationException("transactionContextScope instances must be disposed of in the order in which they were created!");
            ...
            if (_parentScope != null)
            {
                if (_parentScope._disposed)
                {
                     ...
                     * a new scope in the parallel task (with ITransactionContextScopeFactory.CreateNew() instead of 
                     * JoinOrCreate()) then no harm has been done. We haven't tried to access the same TransactionContext
                     * instance from multiple threads.
                     * 
                     * - If this was not the case, they probably already got an exception complaining about the same
                     * TransactionContext or ObjectContext being accessed from multiple threads simultaneously (or a related
                     * error like multiple active result sets on a DataReader, which is caused by attempting to execute
                     * several queries in parallel on the same TransactionContext instance). So the code has already blow up.
                     * 
                     * So just record a warning here. Hopefully someone will see it and will fix the code.
                     */

                    var message = @"PROGRAMMING ERROR - When attempting to dispose a transactionContextScope, we found that our parent transactionContextScope has already been disposed! This means that someone started a parallel flow of execution (e.g. created a TPL task, created a thread or enqueued a work item on the ThreadPool) within the context of a transactionContextScope without suppressing the ambient context first. 
...
3) Change the code so that the ambient context is suppressed before the parallel task is created. You can do this with ITransactionContextScopeFactory.SuppressAmbientContext() (wrap the parallel task creation code block in this). 
...
" + Environment.StackTrace;
         ...
         * Overview: 
         * 
         * We want our transactionContextScope instances to be ambient within 
         ...
         * we could just store our transactionContextScope instances in a ThreadStatic 
         ...
         * a TheadStatic-based ambient TransactionContext here: http://coding.abel.nu/2012/10/make-the-TransactionContext-ambient-with-unitofworkscope/ 
         * 
         ...
         * in EF6 and .NET 4.5.
         * 
         * So we need a storage place for our transactionContextScope instances 
         ...
         * 
         * There are however two issues with storing our transactionContextScope instances 
         * in the CallContext:
         * 
         ...
         * 
         * In our case, our transactionContextScope instances aren't serializable. And in any case,
         ...
         * Instead of storing our transactionContextScope instances themselves in the CallContext,
         * we'll just generate a unique key for each instance and only store that key in 
         * the CallContext. We'll then store the actual transactionContextScope instances in a static
         * Dictionary against their key. 
         * 
         * That way, if an app domain boundary is crossed, the keys will be flown accross
         * but not the transactionContextScope instances since a static variable is stored at the 
         * app domain level. The code executing in the other app domain won't see the ambient
         * transactionContextScope created in the first app domain and will therefore be able to create
         * their own ambient transactionContextScope if necessary.
         * 
         * 2) The CallContext is flow through *all* async points. This means that if someone
         * decides to create multiple threads within the scope of a transactionContextScope, our ambient scope
         * will flow through all the threads. Which means that all the threads will see that single 
         * transactionContextScope instance as being their ambient TransactionContext. So clients need to be 
         * careful to always suppress the ambient context before kicking off a parallel operation
         * to avoid our TransactionContext instances from being accessed from multiple threads.
         * 
         */

        private static readonly string AmbienttransactionContextScopeKey = "AmbienTTransactionContext_" + Guid.NewGuid();

        // Use a ConditionalWeakTable instead of a simple ConcurrentDictionary to store our transactionContextScope instances 
        // in order to prevent leaking transactionContextScope instances if someone doesn't dispose them properly.
        //
        // For example, if we used a ConcurrentDictionary and someone let go of a transactionContextScope instance without 
        ...
        // to the transactionContextScope instances we store in there, allowing them to get GCed.
        ...
        private static readonly ConditionalWeakTable<InstanceIdentifier, TransactionContextScope> transactionContextScopeInstances = new ConditionalWeakTable<InstanceIdentifier, TransactionContextScope>();

        private InstanceIdentifier _instanceIdentifier = new InstanceIdentifier();

        /// <summary>
        /// Makes the provided 'transactionContextScope' available as the the ambient scope via the CallContext.
        /// </summary>
        internal static void SetAmbientScope(TransactionContextScope newAmbientScope)
        {
            if (newAmbientScope == null)
                throw new ArgumentNullException("newAmbientScope");

            var current = CallContext.LogicalGetData(AmbienttransactionContextScopeKey) as InstanceIdentifier;
            ...
            CallContext.LogicalSetData(AmbienttransactionContextScopeKey, newAmbientScope._instanceIdentifier);

            // Keep track of this instance (or do nothing if we're already tracking it)
            transactionContextScopeInstances.GetValue(newAmbientScope._instanceIdentifier, key => newAmbientScope);
        }

        /// <summary>
        /// Clears the ambient scope from the CallContext and stops tracking its instance. 
        /// Call this when a transactionContextScope is being disposed.
        /// </summary>
        internal static void RemoveAmbientScope()
        {
            var current = CallContext.LogicalGetData(AmbienttransactionContextScopeKey) as InstanceIdentifier;
            CallContext.LogicalSetData(AmbienttransactionContextScopeKey, null);

            // If there was an ambient scope, we can stop tracking it now
            if (current != null)
            {
                transactionContextScopeInstances.Remove(current);
            }
        }
        ...
        /// </summary>
        internal static TransactionContextScope GetAmbientScope()
        {
            try
            {
                // Retrieve the identifier of the ambient scope (if any)
                var instanceIdentifier = CallContext.LogicalGetData(AmbienttransactionContextScopeKey) as InstanceIdentifier;
                ...
                // Retrieve the transactionContextScope instance corresponding to this identifier
                TransactionContextScope ambientScope;
                if (transactionContextScopeInstances.TryGetValue(instanceIdentifier, out ambientScope))
                    return ambientScope;

                // We have an instance identifier in the CallContext but no corresponding instance
                // in our transactionContextScopeInstances table. This should never happen! The only place where
                // we remove the instance from the transactionContextScopeInstances table is in RemoveAmbientScope(),
                // which also removes the instance identifier from the CallContext. 
                //
                // There's only one scenario where this could happen: someone let go of a transactionContextScope 
                ...
                // in our transactionContextScopeInstances table. But since we use a ConditionalWeakTable to store 
                // our transactionContextScope instances and are therefore only holding a weak reference to these instances, 
                ...
                System.Diagnostics.Debug.WriteLine("Programming error detected. Found a reference to an ambient transactionContextScope in the CallContext but didn't have an instance for it in our transactionContextScopeInstances table. This most likely means that this transactionContextScope instance wasn't disposed of properly. transactionContextScope instance must always be disposed. Review the code for any transactionContextScope instance used outside of a 'using' block and fix it so that all transactionContextScope instances are disposed of.");
                return null;
            }
            catch (Exception)
            {
