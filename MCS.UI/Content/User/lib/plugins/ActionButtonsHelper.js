...
function ConfirmAction(actionUrl, divUpdateContent, paramObj, contentType, onAjaxSuccess, onAjaxFaild, confirmMsg, actionType, formId, buttonId, confirmResource, cancelResource, clientFunctionToCall, notValidFunctionCall)
{
    ...
    $.confirm({
        ...
        confirmButtonClass: 'btn-site confirmed-btn',
        ...
        confirm: function ()
        {
            $('body > div.jconfirm.white > div.modal-dialog > div > div.modal-footer.actions-buttons.buttons > button.btn-site.confirmed-btn').attr("disabled", true);
            debugger;
            $("#Loader").show();
            event.preventDefault();
            event.stopPropagation();
            // trigger office online save before submit
            if ($('#officeIframe').length > 0)
            {
                var ifrmURL = $('#officeIframe').attr('src');
                $('#officeIframe').attr('src', '');
                //setTimeout(function () {
                //    $('#officeIframe').attr('src', ifrmURL);
                //}, 2000);
            }
