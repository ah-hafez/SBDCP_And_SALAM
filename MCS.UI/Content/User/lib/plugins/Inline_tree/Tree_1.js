...
function FillDepartmentTreeControls(container, id)
{
...
$(document).on("keyup", ".txtDepartmentNumber", function ()
{
    ...
    if ($(this).parents(".externalParties").length == 0)
    {
        ...
        jqXHR = $.ajax(options).done(function (data)
        {
            if (data.RowsCount > 0 && data.Parties.length > 0)
            {
                ...
                selectViaAutoComplete(id, text, txtvalue, container);
            }
            else
            {
                $("#divAutoComplateMenu").html('');
            }
        }).fail(function (jqXHR, textStatus, error)
        {
        });
    ...
    if ($(this).parents(".externalParties").length == 0)
    {
        ...
        jqXHR = $.ajax(options).done(function (data)
        {
            if (data.RowsCount > 0)
            {
                ...
                $("#divContainer__" + container).after("<div id='divAutoComplateMenu' class='single-tree' style='height: 400px; overflow-y: scroll;'></div>");
                var ul = "";
                $(data.Parties).each(function (index, item)
                {
                    ul += "<div onclick=\"selectViaAutoComplete(" + item.Id + ", '" + item.Name + "', '" + item.Number + "', '" + container + "')\" style=\"cursor:pointer\">" + item.Name + "</div>"
                });
    ...
    else
    {
        ...
        jqXHR = $.ajax(options).done(function (data)
        {
            if (data.RowsCount > 0)
            {
                ...
                $(data.Parties).each(function (index, item)
                {
                    ul += "<div onclick=\"selectViaAutoComplete(" + item.Id + ", '" + item.LocalName + "', '" + item.Number + "', '" + container + "')\" style=\"cursor:pointer\">" + item.LocalName + "</div>"
                });
...
function selectViaAutoComplete(id, name, number, container)
{
    //3
    $('#divForm__' + container + ' .hdnDepartmentId').val(id);
    $('#divForm__' + container + ' .hdnDepartmentId').trigger('change');
    $("#divForm__" + container).find(".txtDepartmentName").val(name).trigger;
    $("#divForm__" + container).find('.txtDepartmentNumber').val(number);
    $("#divAutoComplateMenu").remove();

    if (container == "ExternalAssignmentsTree" && document.getElementById("delvMethDiv") !== null)
    {
    ...
    $(".txtDepartmentNumber").trigger('change');
}
...
function SelectTreeNode(treeName, nodeId)
{
    ...
    $('#divDir__' + treeName).find('div.selectable_node').each(function ()
    {
        ...
        if (nodeId == id)
        {
            ...
            $("#divForm__" + treeName).find('.txtDepartmentNumber').val(id);
