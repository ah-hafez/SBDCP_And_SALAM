    ...
    $.extend(true, plugin, {
        ...
        ajaxify: function (options) {
            ...
            if (!self.ajaxified) {
                ...
                if (options.callbackClientFunctionAfter && options.callbackClientFunctionAfter !== '') {
                    self.callbackClientFunctionAfter = options.callbackClientFunctionAfter;
                }
                self.optionalData = {};
                if (options.optionalData && options.optionalData !== '') {
                    self.optionalData = options.optionalData;
                }
                //if (options.callbackClientFunctionBefore && options.callbackClientFunctionBefore !== '') {
                //    self.callbackClientFunctionBefore = options.callbackClientFunctionBefore;
                //}
                if (options.requestType && options.requestType !== '') {
                    self.requestType = options.requestType;
                }

                if (self.gridSort) {
                    if (self.gridSort.indexOf("gridDir=0") != -1) {
                        self.gridSort = self.gridSort.replace("gridDir=0", "gridDir=1");
                    } else {
                        self.gridSort = self.gridSort.replace("gridDir=1", "gridDir=0");
                    }
                ...
                self.getGridUrl = function (griLoaddAction, search, renderRowsOnly) {
                    ...
                    if (self.gridSort) {
                        ...
                        gridQuery.addSearch("gridColumn", mySort["gridColumn"]);
                        gridQuery.addSearch("gridDir", mySort["gridDir"]);
                    }

                    var gridUrl = URI(griLoaddAction).addQuery(gridQuery.search());
                    ...
                    if (URI.decode(griLoaddAction).indexOf("?") != -1) {
                ...
                self.SetupGridHeaderEvents = function () {
                    self.jqContainer.on('click', '.grid-header-title > a[gridname=' + self.jqContainer.data("gridname") + ']', function (e) {
                        var sortedgridName = $(this).attr('gridname');

                        if (sortedgridName == self.jqContainer.data("gridname")) {
                            ...
                            var isAscending = mySearch.indexOf("griDir=1") !== -1;
                            self.gridSort = mySearch.substr(mySearch.match(/gridColumn=\w+/).index);
                            ...
                            if (isAscending) {
                                $(this).attr('href', mySearch.replace("gridDir=1", "gridDir=0"));
                            } else {
                                $(this).attr('href', mySearch.replace("gridDir=0", "gridDir=1"));
                            }
                ...
                self.setupPagerLinkEvents = function () {
                    ...
                    self.jqContainer.off("click", ".grid-next-page");
                    self.jqContainer.on("click", ".grid-next-page", function (e) {
                        e.preventDefault();
                        var pagedgridName = $(this).attr('gridname');

                        if (pagedgridName == self.jqContainer.data("gridname")) {
                            self.currentPage++;
                            self.loadPage();
                    ...
                    self.jqContainer.off("click", ".grid-prev-page");
                    ...
                    self.jqContainer.off("click", ".grid-page-link");
                    ...
                    self.jqContainer.off("click", ".grid-pageSetLink.next");
                    ...
                    self.jqContainer.off("click", ".grid-pageSetLink.prev");
                ...
                self.loadPage = function () {
                    ...
                    var gridTableBody = self.jqContainer.find(".tbody");
                    ...
                    var gridQuery = self.getGridUrl(self.loadPagedDataAction, "", null);
                    if (self.currentPage < 1) {
                        self.currentPage = 1;
                    }
                    var gridUrl = URI(gridQuery).addQuery("page", self.currentPage);
                    gridUrl = encodeURI(URI.decode(gridUrl));

                    $.ajax({
                        url: gridUrl,
                        type: self.requestType,
                        data: self.optionalData,
                        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                        async: true,
                        cache: false,
                        timeout: 20000
                        ...
                        //if (self.callbackClientFunctionBefore != "" && self.callbackClientFunctionBefore != null)
                        //    window[self.callbackClientFunctionBefore]();
                        gridTableBody.html("");
                        ...
                        if (self.callbackClientFunctionAfter != "" && self.callbackClientFunctionAfter != null) {
                            for (var i = 0; i < self.callbackClientFunctionAfter.length; i++) {
                                window[self.callbackClientFunctionAfter[i]]();
                            }
                        }
                    }).fail(function () {
        ...
        clearGridFiltersAndSorting: function () {
            var self = this;
            self.gridColumnFilters = null;
            self.gridSort = self.jqContainer.find("div.sorted a").attr('href', '');
        }
