				...
				if (this.settings[extensionName]) {
					...
					if (typeof oursettings !== 'function') {
						...
						oursettings = function(){
						    ...
						    var undoButtonSytle = 'display:none;;margin:25px 11px !important'
                                , $undoButton = $('<button type="button" value="' + this.settings.undoButtonText + '" class="undo_but eraser circle color-blue" style="' + undoButtonSytle + '" />')
                                    .prependTo(this.$controlbarLower)
                                    .append('<i class="icon-signature-bar-eraser"></i>')
