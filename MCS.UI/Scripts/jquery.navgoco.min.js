(function ($) {
    "use strict"; var Plugin = function (el, options, idx) { this.el = el; this.$el = $(el); this.options = options; this.uuid = this.$el.attr('id') ? this.$el.attr('id') : idx; this.state = {}; this.init(); return this }; Plugin.prototype = {
        init: function () { var self = this; self._load(); self.$el.find('ul').each(function (idx) { var sub = $(this); sub.attr('data-index', idx); if (self.options.save && self.state.hasOwnProperty(idx)) { sub.parent().addClass(self.options.openClass); sub.show() } else if (sub.parent().hasClass(self.options.openClass)) { sub.show(); self.state[idx] = 1 } else { sub.hide() } }); var caret = $('<span></span>').prepend(self.options.caretHtml); var links = self.$el.find("li > div"); self._trigger(caret, !1); self._trigger(links, !0); self.$el.find("li:has(ul) > div").prepend(caret) }, _trigger: function (sources, isLink) {
            ...
            var result = {}, parent = sub.parent(), parents = parent.parents('ul'); parents.each(function () {
                var par = $(this), idx = par.attr('data-index'); if (!idx) { return !1 }
            ...
            var self = this, length = arguments.length; if (length <= 1) { self.$el.find('ul').each(function () { var sub = $(this); self._toggle(sub, open) }) } else {
                var idx, list = {}, args = Array.prototype.slice.call(arguments, 1); length--; for (var i = 0; i < length; i++) { idx = args[i]; var sub = self.$el.find('ul[data-index="' + idx + '"]').first(); if (sub) { list[idx] = sub; if (open) { var parents = self._parents(sub, !0); for (var pIdx in parents) { if (!list.hasOwnProperty(pIdx)) { list[pIdx] = parents[pIdx] } } } } }
        ...
        if (typeof options === 'string' && options.charAt(0) !== '_' && options !== 'init') { var callback = !0, args = Array.prototype.slice.call(arguments, 1) } else { options = $.extend({}, $.fn.navgoco.defaults, options || {}); if (!$.cookie) { options.save = !1 } }
    ...
    }; var cookie = null; $.fn.navgoco.defaults = { caretHtml: '', accordion: !1, openClass: 'open', save: !0, cookie: { name: 'navgoco', expires: !1, path: '/' }, slide: { duration: 400, easing: 'swing' }, onClickBefore: $.noop, onClickAfter: $.noop, onToggleBefore: $.noop, onToggleAfter: $.noop }
})(jQuery)