...
namespace MCS.UI.Areas.User.Controllers
{
    ...
    public class VipInboundController : TransactionController
    {
        ...
        public ActionResult Edit(string id, string defaultTabId)
        {
            try
            {
                ...
                var editInboundDTO = HttpClientWrapper<GetResult<EditInboundDTO>>.GetItemRequest($"api/Vip/GetTransaction?transactionId={trxId}&orgUnitId={SessionInfo.OrgUnitId}&cultureName={SessionInfo.CultureShortName}").Result;
                if (editInboundDTO.StatusCode != StatusCode.Ok)
                {
                ...
                VIPEditInboundVM editInboundVM = EditInboundMapper.VIPMap(editInboundDTO.Result);


                Initialize(editInboundVM.SavedTransactionAssignment);
                InitializerAssignmentPaperData(trxId);

                ViewData["transactionId"] = editInboundVM.Id;
                List<TransactionArchiveVM> transactionArchiveVMs = FillTransactionArchiveVMs(editInboundVM);
                ...
                ViewData["ConfidentialityId"] = editInboundVM.InboundBasicInfoEdit.ConfidentialityLevelId;
                ViewData["ArchiveListData"] = editInboundVM?.Archives != null ? editInboundVM.Archives.ToList() : new List<TransactionArchiveVM>();
                ...
                editInboundVM.InboundBasicInfoEdit.ToFollowUp = false;
                editInboundVM.Id = trxId;
                ViewData["LinkTransactions"] = editInboundVM.Links.ToList();

                Session["TransactionId"] = trxId;

                return View(editInboundVM);
            }
            catch (Exception)
            {
                throw;
        ...
        public ActionResult Send(VipInboundUpdateVM inboundEditVM, string explanationTxt, string selectedConfidentiality)
        {
            try
            {
                ...
                #region 
                byte[] data = DocumentViewerHelper.GetPDFFile(inboundEditVM.hdnMainDocTokenId);
                inboundEditVM.AssignmentVMs.Where(x => x.ActionId == 0).ForEach(x => x.ActionId = inboundEditVM.AssignmentVMs.Where(a => a.ActionId > 0).FirstOrDefault().ActionId);
                ...
                {
                    inboundEditVM.ExplanationForAssignmentPaper = explanationTxt;
                }
                var EditedInbound = EditInboundMapper.VIPEditMap(inboundEditVM);
                if (inboundEditVM.PublicFollowUps != null && inboundEditVM.PublicFollowUps.IsValid())
                {
                   ...
                   HttpClientWrapper<PostResult>.PostRequest(string.Format("api/Transaction/getFollowUpDepartment?EntityId={0}", SessionInfo.OrgUnitId), null).Result;
                    ...
                    {
                        EditedInbound.PublicFollowUps.FollowUpEntityId = (int)postResultFodept.Id;
                ...
                if (inboundEditVM.AssignmentVMs.Where(ta => ta.IsAssigned == true).Count() <= 0)
                {
                ...
                PostResult postResult = HttpClientWrapper<PostResult>
                    .PostRequest(string.Format("api/Transaction/CheckUserHasPermission?sTransactionsIds={0}", inboundEditVM.InboundId.ToString().Trim(',')), TransactionAssignmentMapper.VipMap(inboundEditVM.AssignmentVMs.Where(ta => ta.IsAssigned == true).ToList(), inboundEditVM.ExplanationForAssignmentPaper, "")).Result;
                bool hasPermission = SessionInfo.CurrentUser.Claims.Contains(UserClaims.GeneralPermissions.AssignTransactionToUnauthorize);

                if (postResult.StatusCode != StatusCode.Ok && !inboundEditVM.IsConfirmed)
                {
                ...
                postResult = HttpClientWrapper<PostResult>.PostRequest(string.Format("api/Vip/SaveInbound"), EditedInbound).Result;
                ...
                if (!string.IsNullOrWhiteSpace(explanationTxt))
                {
                    ...
                    PostResult postExplanationResult = HttpClientWrapper<PostResult>
                        .PostRequest(string.Format("api/Transaction/AddTransactionExplanation?transactionId={0}", EditedInbound.Id),
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult InboundAssignItBack(VipInboundUpdateVM inboundEditVM)
        {
            string message = string.Empty;
            ...
            try
            {
               ...
               HttpClientWrapper<GetResult<UserPreferenceDTO>>.GetItemRequest(string.Format("api/UserProfile/GetUserPreference?userId={0}&cultureName={1}&orgUnitId={2}", SessionInfo.CurrentUser.Id, SessionInfo.CultureShortName, SessionInfo.OrgUnitId)).Result;
                ...
                if (userPreferenceVMS != null)
                {
                    ...
                    ViewData["FollowUpProccess"] = GetFollowUpProccess(TransactionCategory.Inbound);
                    ViewData["FollowupPeriod"] = FollowupPeriod(TransactionCategory.Inbound);
                }
                ...
                ViewData["PrioritiesData"] = GetPriorities(TransactionCategory.Inbound);
                ViewData["PrivecyLevelsData"] = TransactionHelper.GetPrivecyLevels(TransactionCategory.Inbound);
                //ViewData["FollowUpDepartmentsData"] = UIHelper.BulidTree(OrgUnitMapper.Map(orgUnitDTOs.Result));
                ViewData["ControllerName"] = "VipInbound";
...
assignmentSaved : transactionAssignmentVMs;
                ...
                IAjaxGrid gridAssignmentPaper = (AjaxGrid<VIPTransactionAssignmentVM>)new AjaxGridFactory().CreateAjaxGrid(transactionAssignmentVMs.OrderBy(x => x.OrderNo).ToList(), 1, 0, true);
        ...
        private List<TransactionArchiveVM> FillTransactionArchiveVMs(VIPEditInboundVM editInboundVM)
        {
            Session["DocoNutDocument"] = null;
