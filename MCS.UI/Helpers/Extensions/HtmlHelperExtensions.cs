...
using YESSER.NCS.Framework.Controls.Mvc;
using YESSER.NCS.Framework.Controls;
using Newtonsoft.Json;
using YESSER.NCS.MCS.Common;
...
using YESSER.NCS.MCS.Common.ApiControllerResults;
using YESSER.NCS.Framework.Localization;
using YESSER.NCS.Framework.Controls.DocumentViewer;
using YESSER.NCS.MCS.Common.CustomAttributes;

using System.Reflection;

namespace YESSER.NCS.MCS.UI 
{
    public static class HtmlHelperExtensions
    {
            ...
            int maxLengthText = 40)
        {
            ...
            GetResult<IList<LookupDTO>> lookups = LookupsHelper.GetLookupItems(lookupCategory, SessionInfo.CultureShortName);
            ...
            if (lookupCategory == LookupCategory.Year)
            {
                ...
                lookups.Result = lookups.Result.ToList().Where(l => Convert.ToInt32(l.Text) <= year).ToList();
            }
            ...
            {
                foreach (LookupDTO lookupDTO in lookups.Result)
                {
                    if (lookupDTO.Text == selectedId)
                    {
                        selectedValue = lookupDTO.Id.ToString();
                    }

                    dataSource.Add(new AutoCompleteDataSource()
                    {
                        Value = lookupDTO.Id.ToString(),
                        Label = lookupDTO.Text
                    });
                }
            }
            ...
            return AutoCompleteExtensions.AutoComplete(html, autoCompleteControlid, hdnIdToSaveValue, items, matchAnywhere, selectedValue, inputClass, ulClassName, buttonId, null, selectFirstIndex, validationClass, waterMarkText);
        }
        ...
        public static MvcHtmlString Tree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, string validationGroup = "")
        {
            ...
            HtmlHelperExtensions.TreeNode(helper, treeViewModel.RootNode, expression, ref sb);
            sb.Append("</ul>");
            sb.Append("</div>");
            ...
            foreach (var node in treeNode.Childs)
            {
                ...
                if (node != null)
                {
                    ...
                    if (node.Childs.Count > 0)
                    {
                        ...
                        HtmlHelperExtensions.TreeNode(helper, node, expression, ref sb);
                        sb.Append("</ul>");
                        sb.Append("</li>");
                    }
        ...
        public static MvcHtmlString PermissionsTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string treeId, bool isAjaxCall, bool enableChildsSelectionByParent = false, string validationGroup = "", string divContainerId = "")
        {
            ...
            {
                return HtmlHelperExtensions.PermissionsMultipleTree(helper, treeViewModel, expression, treeMode, treeId, isAjaxCall, validationGroup);
            }
            else
            {
                return HtmlHelperExtensions.PermissionsSingleTree(helper, treeViewModel, expression, treeMode, treeId, isAjaxCall, validationGroup, divContainerId);
            }
        }
        ...
        private static MvcHtmlString PermissionsSingleTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string container, bool isAjaxCall, string validationGroup = "", string divContainerId = "")
        {
            ...
            sb.Append("<ul class='tree' >");
            HtmlHelperExtensions.PermissionsTreeNode(treeViewModel.RootNode, propertyName, treeMode, container, ref sb);
            sb.Append("</ul>");
            sb.Append("</div>");
            ...
            foreach (var node in treeNode.Childs)
            {
                ...
                if (node != null)
                {
                    ...
                    if (node.Childs.Count > 0)
                    {
                        ...
                        HtmlHelperExtensions.PermissionsTreeNode(node, propertyName, treeMode, container, ref sb);
                        sb.Append("</ul>");
                        sb.Append("</li>");
                    }
        ...
        public static MvcHtmlString PermissionsEditableTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string treeId, bool isAjaxCall, bool enableChildsSelectionByParent = false, string validationGroup = "", string divContainerId = "")
        {
            if (treeMode == TreeMode.Multiple)
            {
                return HtmlHelperExtensions.PermissionsMultipleEditableTree(helper, treeViewModel, expression, treeMode, treeId, isAjaxCall, validationGroup);
            }
            else
            {
                return HtmlHelperExtensions.PermissionsSingleEditableTree(helper, treeViewModel, expression, treeMode, treeId, isAjaxCall, validationGroup, divContainerId);
            }
        }
        ...
        private static MvcHtmlString PermissionsSingleEditableTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string container, bool isAjaxCall, string validationGroup = "", string divContainerId = "")
        {
            ...
            sb.Append("<div class='group_top_directory'>");
            sb.Append("<ul class='tree' >");
            HtmlHelperExtensions.PermissionsEditableTreeNode(treeViewModel.RootNode, propertyName, treeMode, container, ref sb);
            sb.Append("</ul>");
            sb.Append("</div>");
            sb.Append("<div class='col-md-3'></div>");
            sb.Append("</div>");
            ...
            foreach (var node in treeNode.Childs)
            {
                ...
                if (node != null)
                {
                    ...
                    if (node.Childs.Count > 0)
                    {
                        ...
                        sb.Append("</div>");
                        sb.Append("<ul id='ul_" + node.Id + container + "' style='display:none'>");
                        HtmlHelperExtensions.PermissionsEditableTreeNode(node, propertyName, treeMode, container, ref sb);
                        sb.Append("</ul>");
                        sb.Append("</li>");
                    }
        ...
        public static MvcHtmlString DepartmentsTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string treeId, bool isAjaxCall, string onClickfunction = "", string validationGroup = "", string divContainerId = "")
        {
            if (treeMode == TreeMode.Multiple)
            {
                return HtmlHelperExtensions.DepartmentsMultipleTree(helper, treeViewModel, expression, treeMode, treeId, isAjaxCall, onClickfunction, validationGroup);
            }
            else
            {
                return HtmlHelperExtensions.DepartmentsSingleTree(helper, treeViewModel, expression, treeMode, treeId, isAjaxCall, onClickfunction, validationGroup, divContainerId);
            }
        }
        ...
        private static MvcHtmlString DepartmentsSingleTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string treeId, bool isAjaxCall, string onClickfunction = "", string validationGroup = "", string divContainerId = "")
        {
            ...
            {
                HtmlHelperExtensions.DepartmentsTreeNode(treeViewModel.RootNode, propertyName, treeMode, treeId, onClickfunction, ref sb);
            }
            sb.Append("</ul>");
            sb.Append("</div>");
            sb.Append("<div class='col-md-3'></div>");
        ...
        public static MvcHtmlString DepartmentsTreeOnDemand<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string treeId, bool isAjaxCall, string onClickfunction = "", string validationGroup = "", string divContainerId = "", bool isRefreshEnabled = false)
        {
            ...
            if (treeViewModel.RootNode != null)
            {
                HtmlHelperExtensions.DepartmentsTreeNodeOnDemand(treeViewModel.RootNode, propertyName, treeMode, treeId, onClickfunction, ref sb, isRefreshEnabled);
            }
            sb.Append("</ul>");
            sb.Append("</div>");
            sb.Append("<div class='col-md-3'></div>");
        ...
        private static MvcHtmlString DepartmentsMultipleTree<TModel, TProperty>(this HtmlHelper<TModel> helper, TreeViewModel treeViewModel, Expression<Func<TModel, TProperty>> expression, TreeMode treeMode, string container, bool isAjaxCall, string onClickfunction = "", string validationGroup = "")
        {
            ...
            sb.Append("<div class='group_top_directory'>");
            sb.Append("<ul class='tree' >");
            HtmlHelperExtensions.DepartmentsTreeNode(treeViewModel.RootNode, propertyName, treeMode, container, onClickfunction, ref sb);
            sb.Append("</ul>");
            sb.Append("</div>");
            sb.Append("<div class='col-md-3'></div>");
            sb.Append("</div>");
                ...
                foreach (var node in treeNode.Childs)
                {
                    ...
                    if (node != null)
                    {
                        ...
                        if (node.Childs.Count > 0)
                        {
                            ...
                            sb.Append("</div>");
                            sb.Append("<ul id='ul_" + node.Id + container + "' style='display:none'>");
                            HtmlHelperExtensions.DepartmentsTreeNode(node, propertyName, treeMode, container, onClickfunction, ref sb);
                            sb.Append("</ul>");
                            sb.Append("</li>");
                        }
                ...
                foreach (var node in treeNode.Childs)
                {
                    ...
                    if (node != null)
                    {
                        ...
                        if (node.HasChilds)
                        {
                            ...
                            sb.Append("</div>");
                            sb.Append("<ul id='ul_" + node.Id + container + "' style='display:none'>");
                            HtmlHelperExtensions.DepartmentsTreeNodeOnDemand(node, propertyName, treeMode, container, onClickfunction, ref sb);
                            sb.Append("</ul>");
                            sb.Append("</li>");
                        }
        ...
        //public static MvcHtmlString SetValidatation<TModel, TProperty>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TProperty>> expression, string validationGroup = "")
        //{
        //    helper.TextBox("").ToHtmlString();
        //    helper.va
        //    helper.ViewContext.Controller.ViewBag.validationGroup = validationGroup;
        //    return helper.TextBoxFor(expression);
        //}

        //<input data-val="true" data-val-customrequired="Admin.LocalizationDTO.Text" data-val-customrequired-validationgroup="AddUser" id="Names_0__Text" name="Names[0].Text" type="text" value="" class="input-validation-error">
            ...
            foreach (var node in treeNode.Childs)
            {
                ...
                if (node != null)
                {
                    if (node.Childs.Count > 0)
                    {
                        ...
                        HtmlHelperExtensions.NativeTreeNode(node, ref stringBuilder);
                    }
                    else
                    {
        ...
        public static MvcHtmlString MenuLink(this HtmlHelper htmlHelper, string itemText, string actionName, string controllerName, string areaName, string cssClass = "", string permissionCode = "")
        {
            ...
            {
                linkBuilder.MergeAttribute("href", urlHelper.AbsoluteUri + "/" + areaName + "/" + controllerName + "/" + actionName);
            }
        ...
        public static MvcHtmlString DocumentViewer(this HtmlHelper helper,
             string iframeId, AnnotationsMode annotationsMode = AnnotationsMode.ReadOnly, string sessionKey = null, bool withSignAndStamp = false, string signFunctionName = null, bool withBarcode = false, bool withWatermark = false, string watermarkText = "")
        {
            StringBuilder sb = new StringBuilder();

            sb.Append(string.Format("{0}/ArchivingPDF/DocumentViewer.aspx?", UrlHelper.GetBaseUri()));

            sb.Append(string.Format("AnnotationsMode={0}&", (int)annotationsMode));

            if (!string.IsNullOrEmpty(sessionKey))
            {
                sb.Append(string.Format("{0}={1}&", Constants.DocViewerSessionKey, sessionKey));
            }

            if (!string.IsNullOrEmpty(signFunctionName))
            {
                sb.Append(string.Format("SignFunctionName={0}&", signFunctionName));
            }

            sb.Append(string.Format("WithSignAndStamp={0}&", withSignAndStamp));
            sb.Append(string.Format("WithBarcode={0}&", withBarcode));
            sb.Append(string.Format("WithWatermark={0}&", withWatermark));
            sb.Append(string.Format("WatermarkText={0}", watermarkText));

            string iframe = string.Format("<iframe id='{0}' src='{1}' style='width: 100%; height: 800px' frameborder='0'></iframe>", iframeId, sb);
