Create or ALTER FUNCTION [dbo].[GetFullClassificationNameByTransactionId]
(
    @TransactionId INT
)
...
BEGIN
    ...
    DECLARE @Temp TABLE (Level INT IDENTITY(1,1), Item NVARCHAR(500));

    -- Get the first related IC_SUBJECTID for the given transaction
    SELECT TOP 1 @SubjectId = ST.IC_SUBJECTID
    FROM IC_SUBJECTS_TRANSACTION ST
    WHERE ST.TRANSACTIONID = @TransactionId;

    -- Exit if transaction has no subject
    IF @SubjectId IS NULL
        RETURN NULL;

    SET @CurrentId = @SubjectId;

    -- Walk up the hierarchy and store each name in @Temp (child → parent)
    ...
    -- Build the full path (child first)
    SELECT @FullName = 
        STRING_AGG(Item, N' / ') 
        WITHIN GROUP (ORDER BY Level ASC)
    FROM @Temp;
