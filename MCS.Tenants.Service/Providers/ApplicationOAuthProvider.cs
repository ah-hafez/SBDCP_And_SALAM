...
namespace MCS.Tenants.Service.Providers
{
    public class ApplicationOAuthProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            if (string.IsNullOrEmpty(context.Password))
            {
                ...
                user = await MultiTenantUserManager.UserManagerProvider.FindByNameAsync(userName);
            }
            else
            {
                user = await MultiTenantUserManager.UserManagerProvider.FindAsync(context.UserName, context.Password);
            }

            if (user == null)
            {
                context.SetError("UserName Or Password Not Correct");
                return;
            }

            ClaimsIdentity oAuthIdentity = await user.GenerateUserIdentityAsync(MultiTenantUserManager.UserManagerProvider,
